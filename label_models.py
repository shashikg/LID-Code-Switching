...
class EncDecSpeakerLabelModel(ModelPT, ExportableEncDecModel):
    ...
    def __init__(self, cfg: DictConfig, trainer: Trainer = None):
        ...
        self._micro_accuracy = Accuracy(num_classes=num_classes, average='micro', task='multiclass')
    ...
    def training_step(self, batch, batch_idx):
        ...
        for i, top_i in enumerate(top_k):
            self.log(f'training_batch_accuracy_top_{i}', top_i, prog_bar=True)
    ...
    def evaluation_step(self, batch, batch_idx, dataloader_idx: int = 0, tag: str = 'val'):
        ...
        self._micro_accuracy.update(preds=logits, target=labels)
        micro_acc_stats = self._micro_accuracy._final_state()
        ...
        return {
            ...
            f'{tag}_acc_micro_stats': micro_acc_stats,
        }
    ...
    def multi_evaluation_epoch_end(self, outputs, dataloader_idx: int = 0, tag: str = 'val'):
        ...
        self._micro_accuracy.tp = torch.stack([x[f'{tag}_acc_micro_stats'][0] for x in outputs]).sum(axis=0)
        self._micro_accuracy.fp = torch.stack([x[f'{tag}_acc_micro_stats'][1] for x in outputs]).sum(axis=0)
        self._micro_accuracy.tn = torch.stack([x[f'{tag}_acc_micro_stats'][2] for x in outputs]).sum(axis=0)
        self._micro_accuracy.fn = torch.stack([x[f'{tag}_acc_micro_stats'][3] for x in outputs]).sum(axis=0)
        micro_accuracy_score = self._micro_accuracy.compute()

        self._accuracy.reset()
        self._macro_accuracy.reset()
        self._micro_accuracy.reset()

        self.log(f'{tag}_loss', loss_mean, sync_dist=True, prog_bar=True)
        for top_k, score in zip(self._accuracy.top_k, topk_scores):
            self.log(f'{tag}_acc_micro_top_{top_k}', score, sync_dist=True, prog_bar=True)
        self.log(f'{tag}_acc_macro', macro_accuracy_score, sync_dist=True, prog_bar=True)
        self.log(f'{tag}_acc_micro', micro_accuracy_score, sync_dist=True, prog_bar=True)

        return {
            f'{tag}_loss': loss_mean,
            f'{tag}_acc_micro_top_k': topk_scores,
            f'{tag}_acc_macro': macro_accuracy_score,
            f'{tag}_acc_micro': micro_accuracy_score,
        }
